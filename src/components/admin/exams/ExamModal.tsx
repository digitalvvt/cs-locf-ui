...
const ExamModal = ({ open, onClose, examType, initialData }: ExamModalProps) => {
  ...
  // --- Populate courseData whenever 'courses' changes (avoid redundant state updates)
  useEffect(() => {
    const next: GenericType[] = (courses && courses.length > 0)
      ? courses.map((course: any) => ({ id: course.courseId, name: course.name }))
      : [];
    ...
    setCourseData((prev) => {
      if (
        ...
        return prev; // no change; prevent unnecessary re-render loop
      }
      return next;
    });
  }, [courses]);
  ...
  }, [open, isAdmin, adminShift?.id, shiftId, sectionVal, year, semester, programme, getSectionNameFromVal, triggerTargeted]);
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <MultiSelectDropdown
                                disabled={
                                  usersLoading ||
                                  !(Number(year) > 0 && Number(semester) > 0 && Number(programme) > 0 && getSectionNameFromVal() && (isAdmin ? adminShift?.id : shiftId))
                                }
